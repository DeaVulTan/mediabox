...
$Jq.ui = {
	...
	hasScroll: function(el, a) {
		...
		if ($Jq(el).css('overflow') == 'hidden') { return false; }
...
$Jq.extend($Jq.expr[':'], {
	...
	focusable: function(element) {
		var nodeName = element.nodeName.toLowerCase(),
			tabIndex = $Jq.attr(element, 'tabindex');
		return (/input|select|textarea|button|object/.test(nodeName)
			...
			&& !$Jq(element)['area' == nodeName ? 'parents' : 'closest'](':hidden').length;
	},

	tabbable: function(element) {
		var tabIndex = $Jq.attr(element, 'tabindex');
		return (isNaN(tabIndex) || tabIndex >= 0) && $Jq(element).is(':focusable');
	}
});
