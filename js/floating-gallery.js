...
function saveImageOrder(path)
	{
		...
		path = path;
		pars = '&orderId='+orderString;
		new jquery_ajax(path, pars, 'ajaxResultPagings');
		return false;
		//document.getElementById('debug').innerHTML = 'This is the new order of the images(IDs) : <br>' + orderString;
	}

function ajaxResultPagings(data)
	{
		data = unescape(data);
		document.getElementById('selMsgSuccessText').innerHTML = top_friend_updated;
