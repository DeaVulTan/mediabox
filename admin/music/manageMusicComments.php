...
$CFG['lang']['include_files'][] = 'languages/%s/music/admin/manageMusicComments.php';
...
class Comments extends MusicHandler
	{
		...
		public function buildConditionQuery()
			{
				$this->sql_condition = 'music_id=\''.addslashes($this->fields_arr['music_id']).'\'';
			}
		...
		/**
		 * Comments::updateMusicComments()
		 *
		 * @return
		 */
		public function updateMusicComments()
			{
				$sql = 'UPDATE '.$this->CFG['db']['tbl']['music_comments'].
...
$commetns->setFormField('music_id', '');
$commetns->setFormField('cid', '');
$commetns->setFormField('new_cid', '');
$commetns->setFormField('new_comment', '');
/*********** Page Navigation Start *********/
$commetns->setFormField('orderby_field', 'music_comment_id');
$commetns->setFormField('orderby', 'DESC');
$commetns->setTableNames(array($CFG['db']['tbl']['music_comments']));
$commetns->setReturnColumns(array('music_comment_id', 'comment_user_id', 'comment', 'TIMEDIFF(NOW(), date_added) AS date_added', 'comment_status'));
...
if($commetns->isFormPOSTed($_POST, 'updateSubmit'))
	...
	{
		if($commetns->deleteMusicComments($commetns->getFormField('cid')))
			{
				$commetns->setPageBlockShow('block_msg_form_success');
		...
		if($commetns->isResultsFound())
			{
				...
				$commetns->comments_list['hidden_arr'] = array('start', 'music_id');
				$commetns->populateComments();
				$paging_arr = array('music_id');
