...
/* $Id: Smarty.class.php 227 2008-04-14 10:32:29Z selvaraj_47ag04 $ */
...
class Smarty
{
	...
	/**
     * The name of the compiled file name need to encode or not.
     *
     * @var string
     */
	public $smarty_compile_url_encode = false;
 ...
 */
   ...
   /**
    ...
    function changeBasePathForMissingTpl($params)
	{
		global $CFG;
		if($CFG['debug']['validate_tpl_file_available_status'])
			{
				return $params['resource_base_path'];
			}
		$slash = '\\';
		if(strstr($params['resource_base_path'], '/'))
			{
				$slash = '/';
			}
		$search = $slash.'templates'.$slash.$CFG['html']['template']['default'].$slash;
		foreach($CFG['html']['template']['allowed'] as $template)
			{
				$resource_base_path = $params['resource_base_path'];
				$replace = $slash.'templates'.$slash.$template.$slash;
				$file_path = str_replace($search, $replace, $resource_base_path);
				if(is_file($file_path.$slash.$params['resource_name']))
					{
						//echo $file_path.$slash.$params['resource_name'],'<br />';
						return $file_path;
					}
			}
		return $params['resource_base_path'];
    ...
    function _fetch_resource_info(&$params)
    {
        ...
        if(!is_file($_params['resource_base_path'].$_params['resource_name']))
			{
				$_params['resource_base_path'] = $this->changeBasePathForMissingTpl($_params);
			}
    ...
    function _get_auto_filename($auto_base, $auto_source = null, $auto_id = null)
    {
        ...
        if(isset($auto_source)) {
            ...
            $_return .= $this->smarty_compile_url_encode?'%%' . $_crc32 . '%%' . $_filename:$_filename;
        }
