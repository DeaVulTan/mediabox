...
class Zend_Http_Client extends Zend_Http_Client_Abstract
{
    ...
    public function setAuth($user, $password = '', $type = self::AUTH_BASIC)
    {
        ...
        // Else, set up authentication
            ...
            if (! defined('self::AUTH_' . strtoupper($type)))
                return false;//commented by selvaraj//throw new Zend_Http_Exception("Invalid or not supported authentication type: '$auth'");
    ...
    public function setCookiejar($cookiejar = true)
    {
        if ($cookiejar instanceof Zend_Http_Cookiejar) {
            ...
            return false;//commented by selvaraj//throw new Zend_Http_Exception('Invalid parameter type passed as Cookiejar');
        }
    ...
    public function setCookie($cookie, $value = null)
    {
        ...
        if (isset($this->Cookiejar)) {
            if ($cookie instanceof Zend_Http_Cookie) {
                ...
                if (preg_match("/[=,; \t\r\n\013\014]/", $cookie))
                    return false;//commented by selvaraj//throw new Zend_Http_Exception(
    ...
    public function setFileUpload($filename, $formname, $data = null, $ctype = null)
    {
        if (is_null($data)) {
            if (! $data = @file_get_contents($filename))
                return false;//commented by selvaraj//throw new Zend_Http_Exception("Unable to read file '{$filename}' for upload");
    ...
    protected function _prepare_headers()
    {
        ...
        $param_list = array();
        $f_param_list = array_merge($uri_params,$this->paramsGet);
        while (list($param, $value) = each($f_param_list)) {
                array_push($param_list, urlencode($param) . "=" . urlencode($value));
        }
        $query = implode("&", $param_list);
    ...
    protected function _connect()
    {
        ...
        if (! $socket) {
            ...
            return false;//commented by selvaraj//throw new Zend_Http_Exception('Unable to Connect to ' . $this->uri->getHost() . ': ' . $errstr .
                //' (Error Number: ' . $errno . ')');
        }
        ...
        switch ($type) {
            case self::AUTH_BASIC:
                ...
                if (strpos($user, ':') !== false)
                    return false;//commented by selvaraj//throw new Exception("The user name cannot contain ':' in 'Basic' HTTP authentication");
            ...
            default:
                return false;//commented by selvaraj//throw new Zend_Http_Exception("Not a supported HTTP authentication type: '$type'");
        }
