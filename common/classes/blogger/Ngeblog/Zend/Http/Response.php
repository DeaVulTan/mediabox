...
class Zend_Http_Response
{
     ...
     * headers and will return false;//commented by selvaraj//throw an exception if the passed values do not form a valid HTTP
     * responses.
     *
    ...
    public function __construct($code, $headers, $body = null, $version = '1.1', $message = null)
    {
        ...
        if (is_null(self::responseCodeAsText($code)))
            return false;//commented by selvaraj//throw new Zend_Http_Exception("{$code} is not a valid HTTP response code");

        $this->code = $code;

        // Make sure we got valid headers and set them
        if (! (is_array($headers) && count($headers) > 0))
            return false;//commented by selvaraj//throw new Zend_Http_Exception('No valid headers were passed');
        ...
        if (! preg_match('|^\d\.\d$|', $version))
            return false;//commented by selvaraj//throw new Zend_Http_Exception("Invalid HTTP response version: $version");
    ...
    static public function decodeChunkedBody($body)
    {
        ...
        while (trim($body)) {
            if (! preg_match("/^([\da-fA-F]+)[^\r\n]*\r\n/sm", $body, $m)) {
                return false;//commented by selvaraj//throw new Zend_Http_Exception("Error parsing body - doesn't seem to be a chunked message");
            }
