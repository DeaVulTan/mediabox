...
class Zend_Feed_EntryAtom extends Zend_Feed_EntryAbstract
{
     ...
     * does not contain a link rel="edit", we return false;//commented by selvaraj//throw an error (either
    ...
    public function delete()
    {
        ...
        if (!$deleteUri) {
            return false;//commented by selvaraj//throw new Zend_Feed_Exception('Cannot delete entry; no link rel="edit" is present.');
        }
        ...
        if (!$response->isSuccessful()) {
            return false;//commented by selvaraj//throw new Zend_Feed_Exception('Delete failed');
        }
    ...
    public function save($postUri = null)
    {
        if ($this->id()) {
            ...
            if (!$editUri) {
                return false;//commented by selvaraj//throw new Zend_Feed_Exception('Cannot edit entry; no link rel="edit" is present.');
            }
            ...
            if ($client->responseCode !== 200) {
                return false;//commented by selvaraj//throw new Zend_Feed_Exception('Expected response code 200, got ' . $client->responseCode);
            }
        } else {
            if ($postUri === null) {
                return false;//commented by selvaraj//throw new Zend_Feed_Exception('PostURI must be specified to save new entries.');
            }
            $client = Zend_Feed::getHttpClient();
            $client->setUri($postUri);
            $client->post($this->saveXML());
            if ($client->responseCode !== 201) {
                return false;//commented by selvaraj//throw new Zend_Feed_Exception('Expected response code 201, got '
                                             // . $client->responseCode);
            }
        }
        ...
        if (!$newEntry) {
            return false;//commented by selvaraj//throw new Zend_Feed_Exception('XML cannot be parsed: ' . $php_errormsg);
        }

        $newEntry = $newEntry->getElementsByTagName($this->_rootElement)->item(0);
        if (!$newEntry) {
            return false;//commented by selvaraj//throw new Zend_Feed_Exception('No root <feed> element found in server response:'
                                         // . "\n\n" . $client->responseBody);
        }
