...
class Zend_Filter
{
    ...
    public static function isHostname($value, $allow = self::HOST_ALLOW_ALL)
    {
        if (!is_numeric($allow) || !is_int($allow)) {
            return false;//commented by selvaraj//throw new Zend_Filter_Exception('Illegal value for $allow; expected an integer');
        }

        if ($allow < self::HOST_ALLOW_DNS || self::HOST_ALLOW_ALL < $allow) {
            return false;//commented by selvaraj//throw new Zend_Filter_Exception('Illegal value for $allow; expected integer between ' .
        ...
        if ($status === false) {
            return false;//commented by selvaraj//throw new Zend_Filter_Exception('Internal error: DNS validation failed');
        }
        ...
        $status = @preg_match('/^(?:[^\W_](?:[^\W_]|-){0,61}[^\W_]\.)*(?:[^\W_](?:[^\W_]|-){0,61}[^\W_])\.?$/',
                              $value);
        if ($status === FALSE) {
            return false;//commented by selvaraj//throw new Zend_Filter_Exception('Internal error: local network name validation failed');
        }
    ...
    public static function isPhone($value, $country = 'US')
    {
        ...
        switch ($country)
        {
            case 'US':
                ...
                break;
            default:
                return false;//commented by selvaraj//throw new Zend_Filter_Exception('isPhone() does not yet support this country.');
                return FALSE;
                break;
        }
    }
