...
var light_addToCommentHoneyPot = function()
	{
		...
		for (var i=0;i<frm.elements.length;i++)
			{
				var e=frm.elements[i];
				if (e.type!='button' && (e.name).substring(0,8)!='captcha_' && e.name!=arguments[1])
					{
						var ovalue = Trim(e.value);
						if(ovalue)
							{
				...
				if (e.type!='button' && e.name!='captcha_value')
					{
						...
						if(ovalue)
							...
							{
								$('commentSelMsgError').innerHTML=no_comment_error_msg;
								$('selMsgError').show();
								e.value = '';
								e.focus();
								return false;
...
function light_ajaxResultComment(data)
	{
		...
		$Jq('#selEditMainComments').hide('slow');
