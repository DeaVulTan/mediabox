...
function replaceSharePlayEmailFriend(email_address_value, email_friends)
	{
		...
		if(email_friends.checked)
			{
				if(!email_address_value)
					{
						email_address_value = replaceSharePlayEmailFriend(email_address_value, email_friends);
...
function sendAjaxEmail(path, div_id)
	{
		...
		result_div = div_id;
		//For View Playlist
		if(typeof(view_playlist_music_ajax_page_loading) != 'undefined')
			{
				if(view_playlist_music_ajax_page_loading)
					{
						$Jq('#share_submitted').html(view_playlist_music_ajax_page_loading);
						$Jq('#share_loader_row').show();
						if(typeof(recalculate_scroll_view_playlist) != 'undefined')
							{
								if(recalculate_scroll_view_playlist)
									{
										scrollDiv();
									}
							}
		...
		//document.getElementById('selShareMusic').innerHTML = loading;
