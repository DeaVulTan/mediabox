...
function sendAjaxEmail(path, div_id)
	{
		...
		result_div = div_id;
		//For View photo
		if(typeof(view_post_ajax_page_loading) != 'undefined')
			{
				if(view_post_ajax_page_loading)
					{
						$Jq('#share_submitted').html(view_post_ajax_page_loading);
		...
		//document.getElementById('selSharePhoto').innerHTML = loading;
...
var sendAjaxEmailHoneyPot = function()//(path, div_id)
	{
		...
		if(!$Jq('#'+arguments[1]) && $(arguments[1]).value!='')
			{
