<?php
class photoList extends photoHandler
{
    public $slideShowUrl;
    public $advanceSearch;
    public $album_name;
    /**
     * photoList::getPageTitle()
     *
     * @return
     */
    public function getPageTitle()
    {
        ...
        switch ($this->fields_arr['pg'])
		{
            ...
            case 'userphotolist':
                ...
                $name = '<a href="'.getUrl('viewprofile','?user='.$name, $name.'/').'">'.ucfirst($name).$this->LANG['photolist_userphotolist_s'].'</a>';
    ...
    /**
     * photoList::showThumbDetailsLinks()
     *
    ...
    public function showPhotoList()
    {
        ...
        while ($row = $this->fetchResultRecord())
	    {
	        ...
	        if ($this->fields_arr['pg'] != 'albumlist')
			{
				...
				elseif($this->fields_arr['advance_keyword']!='' && $this->fields_arr['advance_keyword']!=$this->LANG['photolist_keyword_field'])
					$search_word=$this->fields_arr['advance_keyword'];
				...
				$result_arr[$inc]['location_recorded_word_wrap_not_highlight'] = $row['location_recorded'];
				...
				$result_arr[$inc]['photo_description_word_wrap_not_highlight'] = $row['photo_caption'];
    ...
    /**
     * photoList::getMySubscriptionTags()
     *
     * @return
     */
	...
	/**
	 * photoList::getMySubscriptionUsers()
	 *
	 * @return
	 */
	...
	/**
	 * photoList::getMySubscriptionCategorys()
	 *
	 * @return
	 */
    ...
    /**
     * photoList::updatephotoTagDetails()
     *
    ...
    /**
     * photoList::updateSearchCountAndResultForphotoTag()
     *
     * @param string $tag
     * @return
     */
    ...
    /**
     * photoList::updateSearchCountForphotoTag()
     *
     * @param string $tag
     * @return
     */
    ...
    /**
     * photoList::getMostViewedExtraQuery()
     *
     * @return
     */
    ...
    /**
     * photoList::getMostDiscussedExtraQuery()
     *
     * @return
     */
    ...
    /**
     * photoList::getMostFavoriteExtraQuery()
     *
     * @return
     */
    ...
    /**
     * photoList::getphotoCountForAlbum()
     *
    ...
    /**
     * photoList::populateSubCategories()
     *
     * @return
     */
    ...
    public function advancedFilters()
    {
        ...
        if ($this->isFormPOSTed($_REQUEST, 'advanceFromSubmission') or $this->getFormField('advanceFromSubmission')==1)
		{
			...
			if ($this->getFormField('advance_keyword') != $this->LANG['photolist_keyword_field'] AND $this->getFormField('advance_keyword'))
			{
				$advanced_filters .=' AND ('.getSearchRegularExpressionQueryModified($this->fields_arr['advance_keyword'],'p.photo_tags','OR').getSearchRegularExpressionQueryModified($this->fields_arr['advance_keyword'],'p.photo_title','OR').getSearchRegularExpressionQueryModified($this->fields_arr['advance_keyword'],'p.photo_caption','').')';
				$this->advanceSearch = true;
			}
	...
	/**
     * photoList::chkAdvanceResultFound()
     *
     * @return
     */
	...
	/**
     * photoList::populateRatingImagesForAjax()
     *
     * @return
     */
	...
	/**
     * photoList::getDescriptionForPhotoList()
	...
	/**
     * photoList::getPhotoLocationFromDb()
     *
     * @return
     */
	...
	/**
	 * photoList::populatePlaylist()
	 * slidelist details will be passed to tpl
	 *
	 * @return void
	 */
	public function populatePlaylist()
	{
		global $smartyObj;
		$playlist_info = array();
		$playlist_info['playlistUrl']=$this->CFG['site']['photo_url'].'photoList.php?ajax_page=true&page=playlist';
		$this->playlist_name_notes = str_replace('VAR_MIN', $this->CFG['fieldsize']['photo_playlist_name']['min'], $this->LANG['playlist_name_notes']);
		$this->playlist_name_notes = str_replace('VAR_MAX', $this->CFG['fieldsize']['photo_playlist_name']['max'], $this->playlist_name_notes);
		...
		$smartyObj->assign('playlist_option', 'photo');
		$smartyObj->display('getSlidelist.tpl');
	}
...
$photolist->setPageBlockNames(array('my_photos_form', 'delete_confirm_form', 'featured_confirm_form',
...
$photolist->CFG['admin']['photos']['individual_photo_play'] = true;
$photolist->makeGlobalize($CFG,$LANG);
$photolist->LANG_LANGUAGE_ARR = $LANG_LIST_ARR['language'];
$photolist->ADDED_WITHIN_ARR = $LANG_LIST_ARR['added_within_arr'];
$photolist->setFormField('photo_id', '');
$photolist->setFormField('playlist_id', '');
$photolist->setFormField('album_id', '');
$photolist->setFormField('photo_ext', '');
$photolist->setFormField('action', '');
$photolist->setFormField('action_new', '');
$photolist->setFormField('act', '');
$photolist->setFormField('pg', '');
$photolist->setFormField('default', 'Yes');
$photolist->setFormField('cid', '');
$photolist->setFormField('myphoto', 'No');
$photolist->setFormField('photo_playlist_id', '');
$photolist->setFormField('myfavoritephoto', 'No');
$photolist->setFormField('user_id', '0');
$photolist->setFormField('ajax_page', '');

/**
 * ********** Page Navigation Start ********
 */
$photolist->setFormField('start', '0');
$photolist->setFormField('slno', '1');
$photolist->setFormField('sid', '');
$photolist->setFormField('photo_ids', array());
$photolist->setFormField('numpg', $CFG['photo_tbl']['numpg']);
$condition = '';
$photolist->setFormField('advanceFromSubmission', '');
$photolist->setFormField('photo_category_name', '');
$photolist->setFormField('added_within', '');
$photolist->setFormField('photo_title', '');
$photolist->setFormField('advan_photo_title', '');
$photolist->setFormField('photo_owner', '');
$photolist->setFormField('advan_photo_owner', '');
$photolist->setFormField('photo_tags', '');
$photolist->setFormField('photo_album_name', '');
$photolist->setFormField('advan_photo_album_name', '');
$photolist->setFormField('location', '');
$photolist->setFormField('searchkey', '');
$photolist->setFormField('titles', '');
$photolist->setFormField('view', '');
$photolist->setFormField('rating', '');
$photolist->setFormField('album_id', '');
$photolist->setFormField('tags', '');
$photolist->setFormField('phototracker', '');
$photolist->setFormField('type', '');
$photolist->setFormField('latitude', '');
$photolist->setFormField('longitude', '');
$photolist->setFormField('address', '');
$photolist->setFormField('photo_location_recorded', '');
$photolist->setFormField('thumb', 'no');
$photolist->setFormField('page', '');
$photolist->setFormField('play_list', '');
$photolist->setFormField('playlist_description', '');
$photolist->setFormField('playlist_access_type', 'Public');
$photolist->setFormField('playlist', '');
$photolist->setFormField('playlist_id', '');
$photolist->setFormField('playlist_name', '');
$photolist->setFormField('advance_keyword', '');
$photolist->setFormField('advantage_photo_owner', '');
$photolist->setFormField('advantage_photo_album_name', '');
$photolist->setFormField('advantage_location', '');

if($photolist->getFormField('pg')=='myplaylist')
{
	$photolist->setFormField('playlist_id', '');
}
$photolist->setFormField('checkbox', array());
$photolist->recordsFound = false;
$photolist->setMinRecordSelectLimit($CFG['data_tbl']['min_record_select_limit']);
$photolist->setMaxRecordSelectLimit($CFG['data_tbl']['max_record_select_limit']);
$photolist->setNumPerPageListArr($CFG['data_tbl']['numperpage_list_arr']);
$photolist->setTableNames(array());
$photolist->setReturnColumns(array());
/**
 * *********** page Navigation stop ************
 */
$photolist->setAllPageBlocksHide();
$photolist->setPageBlockShow('my_photos_form');
$photolist->sanitizeFormInputs($_REQUEST);
/*echo '<pre>';
print_r($_POST);
echo '</pre>';*/
if($photolist->getFormField('type')=='update_location')
{
	$photolist->updatePhotoLocation();
}
if($photolist->getFormField('type')=='add_location' || $photolist->getFormField('type')=='update_location')
{
	$photolist->setPageBlockShow('block_add_location');
}
if($photolist->getFormField('type')=='view_location_photos')
{
	$photolist->setPageBlockShow('block_view_location_photos');
}

if($photolist->getFormField('type')=='remove_location')
{
	if($photolist->removePhotoLocation())
	{
		echo '***--***!!!Yes';
	}
}
$photolist->memberLoginPhotoUrl = getUrl('photolist','?mem_auth=true&pg=photonew','photonew/?mem_auth=true', 'members','photo');
if(isAjax())
{
	$photolist->includeAjaxHeaderSessionCheck();
	if($photolist->getFormField('type')=='get_location')
	{
		$photolist->getPhotoLocationFromDb();
		exit;
	}
	if ($photolist->getFormField('action')=='popplaylist')
    {
    	$photolist->checkLoginStatusInAjax($photolist->memberLoginPhotoUrl);
		$photolist->populatePlaylist();
		die();
    }
    if($photolist->getFormField('page') != '')
	{
		if($photolist->getFormField('page') == 'playlist')
		{
			$photolist->setAllPageBlocksHide();
			$photolist->checkLoginStatusInAjax($photolist->memberLoginPhotoUrl);
			# Add photo to playlist
			$flag = 1;
			if($photolist->getFormField('playlist_name')!= '')
			{
				# Check already exist
				if(!$photolist->chkPlaylistExits('playlist_name', $LANG['viewplaylist_tip_alreay_exists']))
				{
					$flag =0;
					echo $photolist->getFormFieldErrorTip('playlist_name');
				}
				...
				if(!$photolist->chkIsValidSize('playlist_name', 'photo_playlist_name', $subject))
				{
					$flag =0;
					echo $photolist->getFormFieldErrorTip('playlist_name');
				}
			...
			if($flag)
			{
				if($photolist->isFormGETed($_POST, 'playlist') && $photolist->chkIsNotEmpty('playlist',''))
				{
					$playlist_id = $photolist->getFormField('playlist');
					echo sprintf($LANG['viewplaylist_successfully_msg'],$photolist->getFormField('playlist_name'));
					echo '#$#'.$playlist_id;
				}
				else
				{
					# Create new playlist
					$playlist_id = $photolist->createPlaylist();
					$photolist->playlistCreateActivity($playlist_id);
					echo sprintf($LANG['viewplaylist_successfully_msg'],$photolist->getFormField('playlist_name'));
					echo '#$#'.$playlist_id;
				}
				# INSERT SONG TO PLAYLIST SONG
				$photo_id = explode(',', $photolist->getFormField('photo_id'));
				for($inc=0;$inc<count($photo_id);$inc++)
					$photolist->insertPhototoPlaylist($playlist_id, $photo_id[$inc]);
			}
		}
	}

    $photolist->includeAjaxFooter();
	exit;
...
if(isset($_REQUEST['action']))
	$photolist->setFormField('action_new', $_REQUEST['action']);
$action_new = $photolist->getFormField('action_new');
$photolist->setFormField('action', $action_new);
$photolist->setFormField('advan_photo_album_name', $photolist->getFormField('advantage_photo_album_name'));
$photolist->setFormField('photo_album_name', $photolist->getFormField('advan_photo_album_name'));
$photolist->setFormField('advan_photo_owner', $photolist->getFormField('advantage_photo_owner'));
$photolist->setFormField('photo_owner', $photolist->getFormField('advan_photo_owner'));
$photolist->setFormField('location', $photolist->getFormField('advantage_location'));
$photolist->setFormField('photo_keyword', $photolist->getFormField('advance_keyword'));
$photoIds='';
if($photolist->getFormField('tags') && !isset($_POST['avd_search']))
{
$photolist->setFormField('photo_keyword', $photolist->getFormField('tags'));
$photolist->setFormField('advance_keyword', $photolist->getFormField('tags'));
}
elseif(isset($_POST['avd_search']) && $photolist->getFormField('advance_keyword')!=$LANG['photolist_keyword_field'])
{
//$photolist->setFormField('tags', $photolist->getFormField('advance_keyword'));
$photolist->setFormField('tags', '');
}
elseif(isset($_POST['avd_search']) && $photolist->getFormField('advance_keyword')==$LANG['photolist_keyword_field'])
{
...
{
	$photolist->setFormField('titles', $_GET['titles']);
	$_REQUEST['titles'] = $_GET['titles'];
}
if($photolist->getFormField('default')== 'Yes' && $photolist->getFormField('pg')== 'photonew' && $photolist->getFormField('tags') == '')
	$photolist->setFormField('pg', '');
elseif($photolist->getFormField('default')== 'No')
	$photolist->setFormField('pg', (isset($_GET['pg']) && !empty($_GET['pg']))?$_GET['pg']:$_REQUEST['pg']);

if($photolist->getFormField('pg')== 'myphotos')
	$photolist->setFormField('myphoto', 'Yes');

if($photolist->getFormField('pg')== 'myfavoritephotos')
	$photolist->setFormField('myfavoritephoto', 'Yes');

if($photolist->getFormField('pg')=='private_photo')
{
	$photolist->setCommonSuccessMsg($LANG['photolist_private_photo']);
	$photolist->setPageBlockShow('block_msg_form_success');
}
if($photolist->getFormField('pg')=='invalid_photo_id')
{
	$photolist->setCommonSuccessMsg($LANG['common_msg_invalid_photo_id']);
	$photolist->setPageBlockShow('block_msg_form_success');
}

//ADDED THE ERROR MESSAGE IF NOT ALLOWED THE photo UPLOAD
if($photolist->getFormField('pg')=='upload_photo')
{
	$photolist->setCommonSuccessMsg($LANG['photolist_photo_upload_error_message']);
	$photolist->setPageBlockShow('block_msg_form_success');
}
if(!isset($_GET['pg']) && $photolist->getFormField('default')== 'No')
		$photolist->setFormField('pg', '');

if(isset($_GET['view']) && !empty($_GET['view']))
	$photolist->setFormField('view', $_GET['view']);

if(!isMember())
	$photolist->savePlaylistUrl = $photolist->is_not_member_url = getUrl('photolist','?pg=photonew','photonew/', '','photo');
else
	$photolist->savePlaylistUrl = $CFG['site']['photo_url'].'createSlidelist.php?action=save_playlist&light_window=1';

$photolist->photomyPlaylistUrl = $CFG['site']['photo_url'].'photolist.php?action=myphotodelete';
$myphoto_deletelist_arr['photo_list_url'] = getUrl('photolist', '?pg='.$photolist->getFormField('pg'), $photolist->getFormField('pg').'/?action=0', '', 'photo');
$pgValue = $photolist->getFormField('pg');
$pgValue = !empty($pgValue)?$pgValue:'photonew';
$Navigation_arr['photo_list_url_0']      = getUrl('photolist', '?pg='.$pgValue.'&action='.$photolist->getFormField('action').'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), $pgValue.'/?action='.$photolist->getFormField('action').'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), '', 'photo');
...
$photoActionNavigation_arr['photo_list_url_0'] = getUrl('photolist', '?pg='.$pgValue.'&action=0'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), $pgValue.'/?action=0'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), '', 'photo');
$photoActionNavigation_arr['photo_list_url_1'] = getUrl('photolist', '?pg='.$pgValue.'&action=1'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), $pgValue.'/?action=1'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), '', 'photo');
$photoActionNavigation_arr['photo_list_url_2'] = getUrl('photolist', '?pg='.$pgValue.'&action=2'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), $pgValue.'/?action=2'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), '', 'photo');
$photoActionNavigation_arr['photo_list_url_3'] = getUrl('photolist', '?pg='.$pgValue.'&action=3'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), $pgValue.'/?action=3'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), '', 'photo');
$photoActionNavigation_arr['photo_list_url_4'] = getUrl('photolist', '?pg='.$pgValue.'&action=4'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), $pgValue.'/?action=4'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), '', 'photo');
$photoActionNavigation_arr['photo_list_url_5'] = getUrl('photolist', '?pg='.$pgValue.'&action=5'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), $pgValue.'/?action=5'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), '', 'photo');
$photoActionNavigation_arr['photo_list_url_6'] = getUrl('photolist', '?pg='.$pgValue.'&action=6'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), $pgValue.'/?action=6'.'&cid='.$photolist->getFormField('cid').'&sid='.$photolist->getFormField('sid'), '', 'photo');
...
if($photolist->getFormField('pg') == 'photomostviewed'
	OR $photolist->getFormField('pg') == 'photomostdiscussed'
		OR $photolist->getFormField('pg') == 'photomostfavorite')
	{
		if(!$photolist->getFormField('action')) $photolist->setFormField('action', '0');
		$sub_page = 'photoMostViewed_'.$photolist->getFormField('action');
		$photoActionNavigation_arr[$sub_page] = ' class="clsActive"';
	}

$smartyObj->assign('photoActionNavigation_arr', $photoActionNavigation_arr);
if($photolist->getFormField('searchkey'))
{
	$photolist->setFormField('avd_search', '1');
}
$start = $photolist->getFormField('start');
if ($photolist->getFormField('cid') && !$photolist->getFormField('sid') && $CFG['admin']['photos']['sub_category'])
{
    $photolist->setPageBlockShow('form_show_sub_category');
}

$memberphotoListCase = array('myphotos','myfavoritephotos');
if (in_array($photolist->getFormField('pg'),$memberphotoListCase) AND !isMember())
{
	Redirect2URL(getUrl('photolist','?pg='.$photolist->getFormField('pg'),$photolist->getFormField('pg').'/','members','photo'));
}
if ($photolist->isFormPOSTed($_POST, 'avd_reset'))
{
	$photolist->setFormField('photo_keyword', '');
	$photolist->setFormField('photo_album_name', '');
	$photolist->setFormField('photo_owner', '');
	$photolist->setFormField('location', '');
}
if ($photolist->isPageGETed($_GET, 'action'))
{
	$photolist->sanitizeFormInputs($_GET);
}
if($photolist->getFormField('act') == 'myphotodelete')
{
	$photos_arr = explode(',', $photolist->getFormField('photo_id'));
	$photolist->myPhotoListDelete($photos_arr, $CFG['user']['user_id']);
}
if($photolist->getFormField('act') == 'myPlaylistphotoDelete')
{
	$photos_arr = explode(',', $photolist->getFormField('photo_id'));
	//$photolist->deleteMyPlaylistphoto($photos_arr, $CFG['user']['user_id']);
}
if($photolist->getFormField('act') == 'myFavoritephotosDelete')
{
	$photos_arr = explode(',', $photolist->getFormField('photo_id'));
	$photolist->myPhotoListDelete($photos_arr, $CFG['user']['user_id']);
}

if ($photolist->isFormPOSTed($_POST, 'yes'))
{
	if ($photolist->getFormField('act') == 'delete')
	{
		$photos_arr = explode(',', $photolist->getFormField('photo_id'));
		$photolist->deletePhotos($photos_arr, $CFG['user']['user_id']);
		$photolist->setCommonSuccessMsg($LANG['photolist_delete_succ_msg_confirmation']);
		$photolist->setPageBlockShow('block_msg_form_success');

	}
	if ($photolist->getFormField('act') == 'set_avatar')
	{
		$photolist->setFeatureThisImage($photolist->getFormField('photo_id'), $CFG['user']['user_id']);
		$photolist->setPageBlockShow('msg_form_error');
		$photolist->setCommonErrorMsg($LANG['photolist_msg_success_set_avatar']);
	}
	if ($photolist->getFormField('act') == 'favorite_delete')
	{
		$photo_id_arr = explode(',', $photolist->getFormField('photo_id'));
		foreach($photo_id_arr as $photo_id)
		{
			$photolist->deleteFavoritePhoto($photo_id, $CFG['user']['user_id']);
		}
	}
	if ($photolist->getFormField('act') == 'playlist_delete')
	{
		$photo_id_arr = explode(',', $photolist->getFormField('photo_id'));
		foreach($photo_id_arr as $photo_id)
		{
			$photolist->deletePlaylistPhoto($photo_id);
		}
	}
	if ($photolist->getFormField('act') == 'set_playlist_thumb')
	{
		$photo_id = $photolist->getFormField('photo_id');
		$photolist->setPlaylistThumbnail($photo_id);
	}
	if ($photolist->getFormField('act') == 'set_album_thumb')
	{
		$photo_id = $photolist->getFormField('photo_id');
		$album_id = $photolist->getFormField('album_id');
		$photolist->updateAlbumProfileImage($photo_id, $album_id);
	}
}

$photolist->category_name = '';
if ($photolist->isShowPageBlock('form_show_sub_category'))
{
    $photolist->populateSubCategories();
    $photolist->category_name = $photolist->getCategoryName();
    $photolist->LANG['photolist_category_title'] = str_replace('{category}', $photolist->category_name, $LANG['photolist_category_title']);
}
$photolist->LANG['photolist_title'] = html_entity_decode($photolist->getPageTitle());
...
$photolist->showThumbDetailsLinks_arr = $photolist->showThumbDetailsLinks($thum_details_arr);
if ($photolist->isShowPageBlock('msg_form_error'))
{
    $photolist->msg_form_error['common_error_msg'] = $photolist->getCommonErrorMsg();
}
if ($photolist->isShowPageBlock('msg_form_success'))
{
    $photolist->msg_form_success['common_error_msg'] = $photolist->getCommonErrorMsg();
}

if ($photolist->isShowPageBlock('my_photos_form'))
{
    /**
     * ***** navigtion continue********
     */
    $photolist->group_query_arr = array('myrecentlyviewedphoto','photomostfavorite','featuredphotolist','photomostviewed','photomostdiscussed','albumlist');
    $photolist->setTableAndColumns();
    $photolist->buildSelectQuery();
    //$photolist->buildConditionQuery($condition);
    $photolist->buildQuery();
    //$photolist->printQuery();

	//if (in_array($photolist->getFormField('pg'), $group_query_arr))
        $photolist->homeExecuteQuery();
    //else
      //  $photolist->executeQuery();
    /**
     * ****** Navigation End *******
     */
     if($photolist->getFormField('tags'))
		{
			$photolist->advanceSearch = true;
		}
    $photolist->my_photos_form['anchor'] = 'anchor';
    $photolist->isResultsFound = $photolist->isResultsFound();
    if ($CFG['feature']['rewrite_mode'] != 'normal')
    ...
    if ($photolist->isResultsFound())
	{
        if ($CFG['feature']['rewrite_mode'] != 'normal')
            ...
            $paging_arr = array('start','album_id', 'cid', 'tags', 'user_id', 'pg', 'action','photo_album_name','photo_owner','photo_keyword','advan_photo_album_name','advan_photo_owner','location','added_within','advanceFromSubmission','view', 'photo_title', 'advan_photo_title', 'titles', 'myphoto', 'myfavoritephoto', 'thumb');
 	    $smartyObj->assign('paging_arr',$paging_arr);
		$smartyObj->assign('smarty_paging_list', $photolist->populatePageLinksPOST($photolist->getFormField('start'), 'seachAdvancedFilter'));
        switch ($photolist->getFormField('pg'))
		{
            case 'useralbumlist':
                if (!$photolist->isMe($photolist->ALBUM_USER_ID= $photolist->chkValidAlbumId()))
                    break;
            case 'myphotos':
                $photolist->setPageBlockShow('check_all_item');
                break;
            case 'myfavoritephotos':
                $photolist->setPageBlockShow('check_all_item');
                break;
        ...
        $photolist->my_photos_form['showPhotoList'] = $photolist->showPhotoList();
        $photoIds=implode('\',\'',$photolist->photo_id_arr);
    }
}
if ($photolist->isShowPageBlock('block_add_location'))
{
	$photolist->includeHeader();
	if($photolist->getFormField('photo_id'))
	{
		$photolist->getPhotoLocation();
	}
    ?>
    ...
    <script type="text/javascript">
        var pageName='<?php echo $photolist->getFormField('page');?>';
		var googleMap
	 	var address
	 	var marker
	 	var geocoder;
	 	...
	 	var markerpin=0;
	 	var locationedit = 0;
	 	var oldaddress = '<?php echo $photolist->getFormField('address');?>';
     	...
     	function initializegoogleMap()
		{
      		...
      		if (GBrowserIsCompatible())
			{
        		...
        		<?php
					if($photolist->getFormField('latitude') == 0 && $photolist->getFormField('longitude') == 0)
					{
				?>
	        	...
	        	<?php
	        		...
	        		{
	        	?>
						center = new GLatLng(<?php echo $photolist->getFormField('latitude');?>,<?php echo $photolist->getFormField('longitude');?>);
						zoomlevel = 13;
				<?php
					}
				?>
				...
				<?php
					if($photolist->getFormField('address')!='' && $photolist->getFormField('latitude')!=0 && $photolist->getFormField('longitude')!=0)
					{
				?>
						$Jq('#selLocationTextBox').css('display', 'none');
						$Jq('#selSeletedArea').html('<?php echo $photolist->getFormField('address');?>');
	        	...
	        	<?php
	        		...
	        		{
	        			if($photolist->getFormField('address')!='')
	        			{
				?>
							$Jq('#selLocationTextBox').css('display', 'none');
							$Jq('#selSeletedArea').html('<?php echo $photolist->getFormField('address');?>');
		...
		function insertLocation(data)
		{
			...
			var photo_list_id = "'"+$Jq('#photo_id_location').val()+"'";
			var hrefphotolocation = "'"+hreflocation+"'";
			var showPhotosInSelectedLocation = 'showPhotosInLocation('+photo_list_id+','+hrefphotolocation+');';
			location_str += '<a href="javascript:void(0);" id="link_location" onclick="'+showPhotosInSelectedLocation+'">'+oldaddress+'</a>';
			if(photolist_obj)
	...
	<?php
		...
		setTemplateFolder('general/', 'photo');
		$smartyObj->assign('photo_id', $photolist->getFormField('photo_id'));
		$smartyObj->display('addPhotoLocation.tpl');
		//$photolist->includeFooter();
		exit;
}

if($photolist->isShowPageBlock('block_view_location_photos'))
{

	if($photolist->getFormField('photo_location_recorded'))
	{
		$location_photo_details=$photolist->getLocationPhotosDetails();
...
total_photos_to_play = <?php echo count($photolist->player_photo_id); ?>;
<?php
	if(count($photolist->player_photo_id) != 0)
	{
		foreach($photolist->player_photo_id as $photo_id_to_play)
		{
...
setTemplateFolder('general/', 'photo');
$smartyObj->display('photoList.tpl');
...
function jumpAndFormsubmit(path)
{
	...
	url=$path_url+'&view='+path+'&titles=<?php echo $photolist->getFormField("titles");?>';
	document.seachAdvancedFilter.action=url;
	document.seachAdvancedFilter.submit();
}
...
<?php
if (isset($CFG['feature']['show_tooltip_description']) AND $CFG['feature']['show_tooltip_description'])
{
?>
showToolTipForDivs();
<?php
...
function populateMyPlayList(url, opt, photo_id)
{
	playlist_opt = opt;
	return openAjaxWindow('false', 'ajaxupdate', 'jquery_ajax', url, 'action=popplaylist&mem_auth=true&photo_id=' + photo_id, 'populateMyPlayListResponse');
}
